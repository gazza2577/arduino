...
****** Modified by Brendan
* 
* In the SerialCommand.cpp tab the 'a','T', and 'Z' functions are being handled by an external UNO connected via the Serial1 comm port
* In the main loop() we had to scan that port and relay anything coming back from the UNO to the Interface connection
* In the setup() we initialize the Serial1 port
* 
* 
DCC++ BASE STATION
...
void setup(){  
  ...
  Serial.flush();
  Serial.begin(115200);          //Serial 1 setup to handle turnout commands
  Serial.flush();
